...
def handle(msg):
    ...
    if command == '/now':
        ...
        Piero_at_home = True
        f = open("Piero_at_home","w")  #apre il file dei dati in write mode, se il file non esiste lo crea
        f.write("IN")  #scrive la info di presence sul file
        f.close()  #chiude il file dei dati e lo salva
    elif command == '/amin': #forza Annamaria a casa
        Annamaria_at_home = True
...
def set_presence(presence_msg):
    global Piero_at_home, Annamaria_at_home, who_is_at_home, how_many_at_home
...
try:
    f = open("Piero_at_home","r")  #apre il file dei dati in read mode
    ...
    Piero_at_home = False  #se il file non e' presente imposto la presence a False

try:
    f = open("Annamaria_at_home","r")  #apre il file dei dati in read mode
    pres=f.read().strip()   #legge la info di presence sul file
    f.close()  #chiude il file dei dati e lo salva
    if pres == "IN":
        Annamaria_at_home = True
